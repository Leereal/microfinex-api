...
import { Role, Permission, RolePermission, UserRoleAssignment, UserPermission, Prisma } from '@prisma/client';
import { ALL_PERMISSIONS, DEFAULT_ROLE_PERMISSIONS, isValidPermission } from '../constants/permissions';
 ...
 */
export async function seedPermissions(): Promise<{ created: number; updated: number }> {
  let created = 0;
  let updated = 0;
 ...
 * Get all permissions
 */
export async function getAllPermissions(module?: string): Promise<Permission[]> {
 ...
 * Create a new role
 */
export async function createRole(data: CreateRoleInput): Promise<RoleWithPermissions> {
 ...
 * Update a role
 */
export async function updateRole(roleId: string, data: UpdateRoleInput): Promise<RoleWithPermissions> {
...
export async function deleteRole(roleId: string): Promise<void> {
  ...
  if (role._count.userRoles > 0) {
    throw new Error(`Cannot delete role with ${role._count.userRoles} assigned users`);
  }
 ...
 */
export async function assignPermissionsToRole(roleId: string, permissionCodes: string[]): Promise<void> {
 ...
 */
export async function removePermissionsFromRole(roleId: string, permissionCodes: string[]): Promise<void> {
  const permissions = await prisma.permission.findMany({
 ...
 */
export async function assignRoleToUser(data: AssignRoleInput): Promise<UserRoleAssignment> {
 ...
 */
export async function getRoleUsers(roleId: string): Promise<{ id: string; email: string; name: string; assignedAt: Date }[]> {
  const assignments = await prisma.userRoleAssignment.findMany({
    where: { 
      roleId,
      isActive: true 
 ...
 */
export async function setUserDirectPermission(data: UserPermissionInput): Promise<UserPermission> {
 ...
 */
export async function removeUserDirectPermission(userId: string, permissionCode: string): Promise<void> {
 ...
 */
export async function getUserDirectPermissions(userId: string): Promise<UserPermission[]> {
...
export async function getUserEffectivePermissions(userId: string): Promise<UserEffectivePermissions> {
  ...
  const permissionMap = new Map<string, { code: string; name: string; module: string; source: 'role' | 'direct' }>();
  ...
  return {
    userId,
    roles: userRoles.filter(r => r.isActive).map(r => ({ id: r.id, name: r.name })),
 ...
 */
export async function seedDefaultRoles(organizationId: string): Promise<Role[]> {
  const roles: Role[] = [];
 ...
 */
export async function getDefaultRole(organizationId: string): Promise<Role | null> {
